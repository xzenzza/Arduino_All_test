...
float TEMP_value = 35.00;
float HUM_value = 28.00;
float TEMP_value_clearance = TEMP_value+3;
float HUM_value_clearance = HUM_value+20;
...
CMMCInterval myInterval1(5000, Control_Function);
CMMCInterval myInterval2(5000, Update_Thingspeak);
CMMCInterval myInterval3(5000, GetJson_Thingspeak);
...
void setup() {
  ...
  Serial.println(ssid);
/*
IPAddress local_ip = {192,168,1,22};   //��駤�� IP
IPAddress gateway={192,168,1,1};    //��駤�� IP Gateway
IPAddress subnet={255,255,255,0};   //��駤�� Subnet
WiFi.config(local_ip,gateway,subnet);   //set������ѧ�����
*/
...
if (isnan(t1) || isnan(t2)) {
  ...
  else
  {
    ...
    if( temp1 >= TEMP_value || temp2 >= TEMP_value){
         On_Temp();
        }
    if( temp1 <= HUM_value || temp2 <= HUM_value){
          Off_Temp();
        }
    }
